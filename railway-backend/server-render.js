...
// РЕАЛЬНЫЕ БЛЮДА ИЗ ВАШЕГО EXCEL ФАЙЛА - ТОЛЬКО ЭТИ ДАННЫЕ!
const realMenuData = [
  {
    id: 1,
...
app.get('/api/test', (req, res) => {
  res.json({ 
    message: '🎯 RENDER.COM СЕРВЕР С РЕАЛЬНЫМИ ДАННЫМИ РАБОТАЕТ!', 
...
app.get('/health', (req, res) => {
  res.json({ 
    status: 'OK', 
    time: new Date().toISOString(),
    message: 'Render.com сервер работает! Реальные данные из Excel файла загружены!',
    platform: 'Render.com'
  });
});
...
app.listen(PORT, () => {
  ...
  console.log('🚀 Платформа: Render.com');
});
