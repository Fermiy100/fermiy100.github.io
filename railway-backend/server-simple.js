// Простейший сервер без зависимостей для Render.com
...
const server = http.createServer((req, res) => {
  ...
  if (path === '/api/test') {
    const response = {
      message: '🎯 RENDER.COM ПРОСТОЙ СЕРВЕР С РЕАЛЬНЫМИ ДАННЫМИ РАБОТАЕТ!',
      time: new Date().toISOString(),
      version: '1.0.0 - ТОЛЬКО РЕАЛЬНЫЕ ДАННЫЕ ИЗ EXCEL',
      dishes_count: realMenuData.length,
      platform: 'Render.com - Simple Server'
    };
    res.writeHead(200);
    res.end(JSON.stringify(response, null, 2));
  } else if (path === '/api/menu') {
    console.log('🎯 RENDER.COM: ВОЗВРАЩАЕМ РЕАЛЬНЫЕ БЛЮДА ИЗ EXCEL ФАЙЛА!');
    ...
    const response = {
      status: 'OK',
      time: new Date().toISOString(),
      message: 'Render.com простой сервер работает! Реальные данные из Excel файла загружены!',
      platform: 'Render.com - Simple Server'
    };
    res.writeHead(200);
    res.end(JSON.stringify(response, null, 2));
...
server.listen(PORT, () => {
  console.log(`🎯 RENDER.COM ПРОСТОЙ СЕРВЕР ЗАПУЩЕН НА ПОРТУ ${PORT}`);
  console.log('🎉 ВСЕ 15 РЕАЛЬНЫХ БЛЮД ИЗ EXCEL ФАЙЛА ЗАГРУЖЕНЫ!');
  console.log('📊 Блюда:', realMenuData.map(d => d.name).join(', '));
  console.log('🚀 Платформа: Render.com - Simple Server');
});
